...
# 🔥 Remove original source column if exists
data = data.drop("pollution_source_", axis=1, errors="ignore")

# ⭐ Create new pollution source column (for demo purpose)
sources = ["Vehicles", "Industry", "Dust", "Waste", "Agriculture"]

data["pollution_source_new"] = np.random.choice(
    sources, size=len(data), replace=True
)
...
city = st.sidebar.selectbox(
    ...
    sorted(data["pollution_source_new"].dropna().unique())
)
...
# Filter data
filtered = data[data["pollution_source_new"] == city]
...
source = filtered["pollution_source_new"].mode()[0]
...
if pollution_level > 200:
    st.error("🚨 Severe Pollution")
elif pollution_level > 120:
...
source_counts = data["pollution_source_new"].value_counts()
